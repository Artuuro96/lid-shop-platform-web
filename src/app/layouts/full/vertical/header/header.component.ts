...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  // Hide certain header elements on authentication routes
  ...
  constructor(
    ...
    this.isAuthRoute = this.router.url.startsWith('/authentication');
    this.router.events
      .pipe(filter((e) => e instanceof NavigationEnd))
      .subscribe((e: NavigationEnd) => {
        this.isAuthRoute = e.urlAfterRedirects.startsWith('/authentication');
      });
  }
  ...
  private updatePageName(url: string) {
    // Find the matching nav item based on the current route
    const currentNavItem = navItems.find(item => {
      // Handle exact matches and partial matches for nested routes
      return url === item.route || url.startsWith(item.route + '/');
    });
    
    if (currentNavItem && currentNavItem.displayName) {
      this.currentPageName = currentNavItem.displayName;
    } else {
